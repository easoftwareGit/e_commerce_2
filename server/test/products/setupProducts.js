...
async function insertAllProducts() {
  const sqlCommand = `
    INSERT INTO ${productsTableName} (uuid, name, category, price, description, designer)
    VALUES ($1, $2, $3, $4, $5, $6) 
    RETURNING *;`;
  try {
    for (let i = 0; i < products.length; i++) {
      const product = products[i];
      const { uuid, name, category, price, description, designer } = product;
      const rowValues = [uuid, name, category, price, description, designer];
